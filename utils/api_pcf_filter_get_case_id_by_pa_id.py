...
def pcf_filter_get_case_id_by_pa_id(r_pa_id):

    hash_case_id = {}

    if r_pa_id != "":
        OBJ_MYSQL = MySQLdb.connect(unix_socket=db_sock, host="localhost", db=db_name, user=db_user, passwd=db_pw, charset="utf8")
        sql_CaseGene = u"select A.gene_id,B.Symbol,B.CaseID from vgp_genes as A, CaseGene as B where A.gene_id=B.EntrezID AND A.panel_id=%s"
