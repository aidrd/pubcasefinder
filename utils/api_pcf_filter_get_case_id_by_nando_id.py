...
def pcf_filter_get_case_id_by_nando_id(r_nando_id):

    hash_case_id = {}

    if r_nando_id != "":
        url_api_pcf_filter_get_gene_id_by_nando_id  = "https://pubcasefinder.dbcls.jp/sparqlist/api/pcf_filter_get_gene_id_by_nando_id"
        dict_param_api_pcf_filter_get_gene_id_by_nando_id  = {"nando_id":r_nando_id}
        r_post_data = requests.post(url_api_pcf_filter_get_gene_id_by_nando_id, data=dict_param_api_pcf_filter_get_gene_id_by_nando_id)
        json_post_data = r_post_data.json()

        # get data for each result
        if r_nando_id in json_post_data and json_post_data[r_nando_id]:
            OBJ_MYSQL = MySQLdb.connect(unix_socket=db_sock, host="localhost", db=db_name, user=db_user, passwd=db_pw, charset="utf8")

            for geneid in json_post_data[r_nando_id]:	
