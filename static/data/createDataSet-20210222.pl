...
while(defined ($line = <FH>)){
    ...
    my $Japanese_term_expert = $fields[2];
    my $Japanese_term_lsd    = $fields[3];
    my $Japanese_term_mpj    = $fields[4];
    my $Japanese_term_google = $fields[5];
...
#    next if($English_term =~ /^obsolete/);

    my $key = "";
    if($Japanese_term_expert ne "NA"){
        $key = $Japanese_term_expert;
    }elsif($Japanese_term_lsd ne "NA"){
        $key = $Japanese_term_lsd;
    }elsif($Japanese_term_mpj ne "NA"){
        $key = $Japanese_term_mpj;
    }elsif($Japanese_term_google ne "NA"){
        $key = $Japanese_term_google;
    }else{
        print STDERR "ERROR: empty line: $ID\n";
        next;
    }

    $key =~ s/^\s+|\s+$//g;
    my $Japanese_term = $key;
#    my $line1 = "$ID\t$English_term\t$def";
