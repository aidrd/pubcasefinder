...
function _onload() {
	...
	$("#btn_paste_sampletext").click(function (){
		let example=EXAMPLE_TEXT.replace(/<br>/g,'\n');
	...
	$("#btn_parse_userinput").on("click", function() {
		...
		if(hpo_dic && hpo_dic.length >0) {
			_parse_text(note);
		}else {
			// try reload json from server
...
function _create_list_from_matches(matches_lst) {
	...
	for( let i=0; i<matches_lst.length; i++) {
		...
		if(!hpo_japanese) hpo_japanese = hpo_english;
...
function _changeHPOforSelectedMatch(hpo_id_in){

	let hpo_id = hpo_id_in.replace('_ja', '');
	...
	for(let i=0; i< hpo_dic.length; i++){
		...
		if(hpo_dic[i].HPO_ID === hpo_id){
			...
			matches_list[SELECTED_MATCH_IDX].def		  = _get_HPO_DEF(hpo_id);
			notfound = false;
			break;
		}
	}
...
function _tsv2JSON(tsv){
	...
	for(let i=1;i<lines.length;i++){
		...
		if(isDefinitionLine){
			...
			_store_HPO_DEF(currentline[0], currentline[1]);
		}else if(lines[i] == "DEFINITION:"){
...
var hpo_dic_def = [];
function _store_HPO_DEF(hpo_id,def) {
	hpo_dic_def[hpo_id] = def;
}

function _get_HPO_DEF(hpo_id){
	if(hpo_id in hpo_dic_def) return hpo_dic_def[hpo_id];
	return null;
}
	...
	$.post(SERVER_POST_URL,"text=" + encodeURI(term_in_text))
		.done(function(data1,textStatus,jqXHR) {
			...
			if(data1 && data1 !== 'none'){
				...
				_create_popuplist(data1.split(','),rangeStart,rangeEnd);
			}else{
...
function _create_popuplist(hpolst_from_server,rangeStart,rangeEnd){
	...
	for(let idx=0; idx < hpolst_from_server.length; idx++){
		...
		let hpo_id = hpolst_from_server[idx].replaceAll('_ja','');
		
		for(let i=0; i< hpo_dic.length; i++){
			if(hpo_dic[i].HPO_ID === hpo_id){
				let obj = {HPO_ID:	   hpo_id,
						   SEARCH_KEY: hpo_dic[i].SEARCH_KEY,
						   JT:		   hpo_dic[i].JT,
						   ET:		   hpo_dic[i].ET,
				...
				$li = $('<li>').appendTo($ul);
				$a  = $('<a>').text(hpo_dic[i].JT + ' ('+ hpo_dic[i].ET +')')
							  .data('HPO_DATA',obj)
							  .click(function(){
								  let obj_data = $(this).data('HPO_DATA');
								  _add_match(obj_data);
							  })
							  .appendTo($li);
				num++;
...
function _add_match_by_tokeninput(hpo_id_in,hpo_name){
	...
	for(let i=0; i< hpo_dic.length; i++){
		if(hpo_dic[i].HPO_ID === hpo_id){
			...
			obj['term_in_dic']  = hpo_id === hpo_id_in ? hpo_dic[i].ET : hpo_dic[i].JT;
			obj['jterm_in_dic'] = hpo_dic[i].JT;
			obj['eterm_in_dic'] = hpo_dic[i].ET;
			obj['def']		    = _get_HPO_DEF(hpo_id);
	...
	if(!('id_in_dic' in obj)){
		alert('indicated hpo ' + hpo_id_in + ' not found in dic data!');
		return;
	}
...
function _add_match(hpo_data) {
	let normalized_text = _get_normalized_text();	
	let term_in_text = normalized_text.substring(hpo_data.rangeStart, hpo_data.rangeEnd + 1);
	
	let matches_list = _get_matches();
	let overlap_lst = _find_overlap(hpo_data.rangeStart,hpo_data.rangeEnd,matches_list);
	if(overlap_lst.length > 0){
	  for(let i=overlap_lst.length-1; i >= 0; i--){
		matches_list.splice(overlap_lst[i],1);
	  }
	...
	let obj = {start:		  hpo_data.rangeStart,
			   end:			  hpo_data.rangeEnd,
			   id_in_dic:	  hpo_data.HPO_ID,
			   term_in_dic:   hpo_data.SEARCH_KEY,
			   jterm_in_dic:  hpo_data.JT,
			   eterm_in_dic:  hpo_data.ET,
			   def:			  _get_HPO_DEF(hpo_data.HPO_ID),
			   term_in_text:  term_in_text,
			   with_symptoms: true
	};
	...
	let send_url_str = "https://pubcasefinder.dbcls.jp/pcf_share?share=annotation&text="+encodeURIComponent(term_in_text)+"&phenotype="+ encodeURIComponent(hpo_data.hpo_id);
	$.ajax({	
		url:	  send_url_str,  // 通信先のURL
		type:	 "GET",// 使用するHTTPメソッド(get/post)
		async:	true,	// 使用するHTTPメソッド(true/false)
	...
	for(let i=0; i<matches_list.length; i++){
		...
		if(start_m === hpo_data.rangeStart && end_m === hpo_data.rangeEnd){
			let element = document.getElementById("li_hpo_" + i);
			if(element) element.scrollIntoViewIfNeeded();
			_choose_match_from_list(i);
			break;
