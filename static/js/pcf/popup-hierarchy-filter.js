;(function ($) {
	...
	var CSS_PREFIX = 'popup-hierarchy-filter-';
	...
	var DEFAULT_SETTINGS = {
		...
		inputId		 : 'popup-hierarchy-filter',
		...
		tokenInputFunctionName	 : 'tokenInput_filter',
		tokenInputListClassName	 : 'TokenList_filter',
	...
	var KEY_PREFIX = 'popupRelationFilter',SETTINGS_KEY = KEY_PREFIX+'Settings',OBJECT_KEY = KEY_PREFIX+'Object';
	...
	var PopupRelationFilter = function (input, url_or_data_or_function, settings) {
		...
		var original_input_id = input.id;
		var original_ul_class = 'ul-' + input.id;
...
//		console.log(tokeninput_settings);
		...
		current_settings.inputId = original_input_id + '-' + current_settings.inputId;
		...
		function getOriginalTokenInputItemNodes(){
			var original_ul_class = 'ul-' + input.id;
			return $('ul.'+original_ul_class+'.'+tokeninput_classes['tokenList'].split(/\s+/).join('.')).not('.'+current_settings.cssTokenListClass).children('li.'+tokeninput_classes['token']).not('.'+current_settings.cssTokenClass).toArray();
		}

		function getOriginalTokenInputItems(){
			var original_input_id = input.id;
			return $.map($('#'+original_input_id)[functionName]('get'), function(data){
...
//			});
		...
		function getTokenInputItemNodes(){
			var ul_class = 'ul-' + current_settings.inputId;
			return $('ul.'+ul_class+'.'+tokeninput_classes['tokenList'].split(/\s+/).join('.')+'.'+current_settings.cssTokenListClass+'>li.'+tokeninput_classes['token']+'.'+current_settings.cssTokenClass).toArray();
		}
		...
		function _addTokenInputItem(token,selectedToken){
			...
			var ul_class = 'ul-' + current_settings.inputId;
			var $li = $(current_settings.nodeName+'.'+current_settings.cssSelectedPhenotypeClass+ ' ul.'+ul_class+'.'+tokeninput_classes['tokenList'].split(/\s+/).join('.')+'.'+current_settings.cssTokenListClass+'>li.'+tokeninput_classes['token']).not('.'+current_settings.cssTokenClass).addClass(current_settings.cssTokenClass);
					...
					.click(function(){
						...
						if(false){
							if(isString(options.classname) && options.classname === CSS_PREFIX+settings.keySubclass){
								$(current_settings.nodeName+'.'+current_settings.cssTableClass).css({'animation':'popup-hierarchy-filter-keyframe-subclass-translate 500ms ease-out 0s normal both'});
							}
							else if(isString(options.classname) && options.classname === CSS_PREFIX+current_settings.keySuperclass){
								$(current_settings.nodeName+'.'+current_settings.cssTableClass).css({'animation':'popup-hierarchy-filter-keyframe-superclass-translate 500ms ease-out 0s normal both'});
							}
							setTimeout(function(){
								runSearch(data.id);
							},500);
...
//						$.PopupRelationFilterTokenTooltip();
					...
					onReady: function(){
						var ul_class = 'ul-' + current_settings.inputId;
						var $ul = $(current_settings.nodeName+'.'+current_settings.cssSelectedPhenotypeClass+ ' ul.'+ul_class+'.'+tokeninput_classes['tokenList'].split(/\s+/).join('.')).addClass(current_settings.cssTokenListClass);
					...
					onHideDropdownItem: function(){
						$.PopupRelationFilterResultsTooltip();
					}
				}));
...
//							.text(value)
		...
		function closeMagnificPopup(){
			...
			if(magnificPopup.contentContainer && $('.mfp-wrap')) {
				let $inlineContentBase = $.magnificPopup.instance.contentContainer.find('div.popup-hierarchy-filter-inline-content-base');
		...
		if(isObject(tokeninput_settings) && tokeninput_settings.classes) {
			if(isObject(tokeninput_classes) && isString(tokeninput_classes['tokenList']) && isString(tokeninput_classes['token'])){
				var original_ul_class = 'ul-' + input.id;
				tokeninput_selector = 'ul.'+original_ul_class+'.' +tokeninput_classes['tokenList'].split(/\s+/).join('.')+
									  '>li.'+tokeninput_classes['token']+'.'+tokeninput_classes['tokenTerm']+'.'+tokeninput_classes['tokenTermPopup'];
				$(document).on('click', tokeninput_selector, onTokenItemClicked);

				var ul_class = 'ul-' + current_settings.inputId;
				var tokeninput_selector2 = 'ul.'+ul_class+'.' +tokeninput_classes['tokenList'].split(/\s+/).join('.')+
											'>li.'+tokeninput_classes['token']+'.'+tokeninput_classes['tokenTerm']+'.'+tokeninput_classes['tokenTermPopup'];
				$(document).on('click', tokeninput_selector2, onTokenItemClicked);
			}
		}

		function onTokenItemClicked(){
					var click_text = '';
					var $li_node;
		...
		$.PopupRelationFilter = function(query, options, settings){
		...
		$.PopupRelationFilterTokenTooltip = function(){
...
//				console.log(top,$(document).children('html').get(0).scrollTop,$('nav.fh5co-nav').outerHeight(true));
		...
		$.PopupRelationFilterResultsTooltip = function(node,token_data,options){
			...
			var $node = $(node);
			var data_key = 'popup-hierarchy-filter-tooltip-data';
...
//			console.log($item.closest('ul').html())
			...
			if(isObject(token_data) && $dropdown.length){
				$.PopupRelationFilterResultsTooltip($dropdown,token_data);
			}
			e.stopPropagation();
			return false;
		});
